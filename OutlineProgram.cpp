...
OutlineProgram1::OutlineProgram1() {
    ...
    " float off = floor(5.0 / (sqrt(dist2) + 1.0) + 1.0);\n"
//    " float off = 1.0;\n"
    ...
    " float dpx0 = dot(normalize(px0.xyz - p), n);\n"
    " float dpx1 = dot(normalize(px1.xyz - p), n);\n"
    " float dpy0 = dot(normalize(py0.xyz - p), n);\n" 
    " float dpy1 = dot(normalize(py1.xyz - p), n);\n"
